...
function AdminProtections() {
  ...
  const fetchProtections = async () => {
    try {
      ...
      const res = await axios.get(
        "http://127.0.0.1:8000/api/admin/protections",
        {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.id) {
        await axios.put(
          `http://127.0.0.1:8000/api/admin/protections/${formData.id}`,
          formData,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
  ...
  return (
    <div className="p-6 md:p-8 font-sans text-slate-800">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Kelola produk proteksi perjalanan.
          </p>
        </div>
        <button
          onClick={() => {
      ...
      <Modal
        ...
        maxWidth="max-w-md"
        title={
          formData.id ? "Edit Layanan Proteksi" : "Tambah Layanan Proteksi"
        }
      >
        <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="Misal: Asuransi Keterlambatan"
            />
          </div>
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-[#1800ad]/100 focus:bg-white transition-colors"
              placeholder="Jaminan 100% uang kembali jika terlambat."
            />
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider pl-6">
                  ID
                </th>
                ...
                <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center pr-6">
                  Aksi
                </th>
              </tr>
            </thead>
                  ...
                  .map((prot) => (
                    ...
                    >
                      ...
                      <td className="p-4 text-sm font-semibold text-slate-800">
                        Rp {parseInt(prot.price).toLocaleString("id-ID")}
                      </td>
                      <td className="p-4">
                        <span
                          className={`inline-block px-2.5 py-1 text-xs font-bold tracking-wide uppercase rounded-lg border ${prot.is_active ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-red-50 text-red-600 border-red-100"}`}
                        >
