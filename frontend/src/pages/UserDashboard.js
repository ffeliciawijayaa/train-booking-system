        ...
        <div className="font-sans text-slate-800 bg-slate-50 min-h-screen pb-0">
            ...
            <div className="relative w-full h-screen bg-slate-900 overflow-hidden">
                ...
                <div className="relative z-10 flex justify-between items-center px-6 md:px-12 py-6">
                    <h1 className="text-2xl font-black text-white tracking-wide">KAI<span className="text-blue-400">Hub</span></h1>
                    <div className="flex gap-4 items-center">
                        {localStorage.getItem('token') ? (
                            <>
                                <button onClick={() => navigate('/my-tickets')} className="hidden sm:block text-white hover:text-blue-200 font-medium text-sm transition">Riwayat Tiket</button>
                                <button onClick={() => navigate('/profile')} className="hidden sm:block text-white hover:text-blue-200 font-medium text-sm transition">Profil</button>
                                <button onClick={handleLogout} className="px-5 py-2.5 bg-white/10 hover:bg-white/20 border border-white/20 text-white rounded text-sm font-bold backdrop-blur-md transition">Keluar</button>
                            </>
                        ) : (
                            <button onClick={() => navigate('/login')} className="px-6 py-2.5 bg-white text-blue-900 hover:bg-slate-100 rounded text-sm font-bold shadow transition">Masuk / Daftar</button>
                        )}
                    </div>
                </div>
                ...
                <div className="relative z-10 flex flex-col items-start justify-start pt-24 md:justify-center md:pt-0 h-full text-left px-6 md:px-28 lg:px-32 max-w-6xl md:-mt-40">
                    <h2 className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-extrabold text-white leading-tight mb-2 md:mb-5 drop-shadow-lg">
                        Jelajahi Nusantara <br className="hidden md:block" /> Dengan Kenyamanan
                    </h2>
                    ...
                    <div className="bg-white rounded shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] p-4 md:p-6 border border-slate-100">
                        <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-3 items-center">
                            <div className="w-full relative bg-slate-50 rounded hover:bg-slate-100 transition-colors">
                                <label className="absolute top-3 left-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Keberangkatan</label>
                                <select
                                    value={originId}
                            ...
                            <div className="w-full relative bg-slate-50 rounded hover:bg-slate-100 transition-colors">
                                <label className="absolute top-3 left-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tujuan</label>
                                <select
                                    value={destinationId}
                            ...
                            <div className="w-full relative bg-slate-50 rounded hover:bg-slate-100 transition-colors">
                                <label className="absolute top-3 left-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tanggal</label>
                                <input
                                    type="date"
                                    value={journeyDate}
                ...
                {hasSearched && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="flex justify-between items-end mb-8">
                            <div>
                                <h3 className="text-2xl font-black text-slate-900">Jadwal Keberangkatan</h3>
                                <p className="text-slate-500 mt-1">Ditemukan {tickets.length} kereta yang melayani rute Anda.</p>
                            </div>
                        </div>
                        ...
                        {loading && (
                            <div className="p-12 text-center bg-white rounded border border-slate-100 shadow-sm">
                                ...
                                <div className="text-slate-500 font-medium">Mencari tiket terbaik...</div>
                            </div>
                        )}
                        ...
                        {!loading && tickets.length === 0 && (
                            <div className="p-16 text-center bg-white border border-slate-100 rounded shadow-sm">
                                ...
                                <h4 className="text-xl font-bold text-slate-800 mb-2">Tiket Tidak Ditemukan</h4>
                                <p className="text-slate-500">Coba ganti stasiun keberangkatan, tujuan, atau tanggal perjalanan Anda.</p>
                            </div>
                        )}
                        ...
                        {!loading && tickets.length > 0 && (
                            <div className="space-y-5">
                                {tickets.map((ticket, index) => (
                                    <div key={index} className="bg-white p-6 sm:p-8 rounded shadow-sm hover:shadow-lg hover:-translate-y-1 border border-slate-100 transition-all duration-300 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                                        <div className="flex-1 w-full">
                                            <div className="flex items-center gap-3 mb-4">
                                                <h4 className="text-xl font-bold text-slate-900">{ticket.train_name}</h4>
                                            ...
                                            <div className="flex items-center gap-4 sm:gap-8">
                                                <div className="min-w-[80px]">
                                                    <div className="text-3xl font-black text-slate-800 tracking-tight">{ticket.departure_time}</div>
                                                    <div className="text-[10px] font-bold text-slate-400 uppercase mt-1">Berangkat</div>
                                                </div>
                                                ...
                                                <div className="text-right min-w-[80px]">
                                                    <div className="text-3xl font-black text-slate-800 tracking-tight">{ticket.arrival_time}</div>
                                                    <div className="text-[10px] font-bold text-slate-400 uppercase mt-1">Tiba</div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="md:border-l border-slate-100 md:pl-8 w-full md:w-auto flex flex-row md:flex-col justify-between items-center md:items-end gap-4 pt-6 md:pt-0 border-t md:border-t-0">
                                            <div className="text-left md:text-right">
                                                <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Harga / Pax</div>
                ...
                {!hasSearched && (
                    <div className="mt-12">
                        <div className="text-center mb-12">
                            <h3 className="text-2xl sm:text-3xl font-black text-slate-900">Kenapa KAIHub?</h3>
                            <p className="text-slate-500 mt-3">Nikmati pengalaman pemesanan tiket kereta api terbaik dengan fitur unggulan kami.</p>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            <div className="bg-white p-8 rounded shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
                                <div className="w-14 h-14 bg-blue-50 text-blue-600 rounded flex items-center justify-center text-2xl mb-6">⚡</div>
                                <h4 className="font-bold text-lg text-slate-900 mb-2">Booking Instan</h4>
                                <p className="text-slate-500 text-sm leading-relaxed">Lewati antrean panjang. Cari, pilih, dan pesan tiket Anda dalam hitungan detik dari mana saja.</p>
                            </div>
                            <div className="bg-white p-8 rounded shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
                                <div className="w-14 h-14 bg-emerald-50 text-emerald-600 rounded flex items-center justify-center text-2xl mb-6">🛡️</div>
                                <h4 className="font-bold text-lg text-slate-900 mb-2">Pembayaran Fleksibel</h4>
                                <p className="text-slate-500 text-sm leading-relaxed">Transaksi aman dengan QRIS, Virtual Account, dan berbagai metode pembayaran terpercaya lainnya.</p>
                            </div>
                            <div className="bg-white p-8 rounded shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
                                <div className="w-14 h-14 bg-amber-50 text-amber-600 rounded flex items-center justify-center text-2xl mb-6">✨</div>
                                <h4 className="font-bold text-lg text-slate-900 mb-2">Perjalanan Nyaman</h4>
                                <p className="text-slate-500 text-sm leading-relaxed">Nikmati fasilitas kelas dunia dengan asuransi perlindungan ekstra untuk perjalanan terbaik Anda.</p>
                            </div>
                        </div>
                    </div>
                )}
            ...
            <footer className="border-t border-slate-200 bg-white py-12 text-center text-slate-500 text-sm mt-auto">
                <div className="font-black text-slate-900 text-xl mb-3">KAI<span className="text-blue-600">Hub</span></div>
